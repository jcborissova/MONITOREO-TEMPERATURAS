...
const ReportPage: React.FC = () => {
  ...
  const handleDateChange = (type: "start" | "end", value: string) => {
    ...
    const rawStart = type === "start" ? value : dateRange.start;
    const rawEnd = type === "end" ? value : dateRange.end;
    const normalized = normalizeRange(rawStart, rawEnd);
  ...
  const reportData: ReportRow[] = useMemo(() => {
    ...
    const rows: ReportRow[] = Object.entries(historyData).map(
      ([zoneKey, measuresAny]) => {
        const measures = Array.isArray(measuresAny)
          ? (measuresAny as Measure[])
          : [];
        ...
        const hums = base
          .map((m: any) =>
            Number(
              m.humedity ?? (m as any).humidity ?? (m as any).hum
            )
          )
          .filter(Number.isFinite);
        ...
        const avg = (arr: number[]) =>
          arr.length
            ? (arr.reduce(sum, 0) / arr.length).toFixed(2)
            : "—";
        ...
        return {
          ...
          "Temp Mín (°C)": temps.length
            ? Math.min(...temps).toFixed(2)
            : "—",
          "Temp Máx (°C)": temps.length
    ...
    const filtered = rows.filter((r) => {
      const passText = text
        ? String(r.Zona).toLowerCase().includes(text)
        : true;
      const passEmpty = hideEmpty
        ? (r["Total Registros"] ?? 0) > 0
        : true;
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <input
                ...
                max={toYMD(new Date())}
                className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none w-full sm:w-44"
              />
      ...
      {reportData.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center text-center bg-white border border-gray-100 rounded-2xl p-8 sm:p-12 shadow-sm">
          ...
          <p className="mt-1 text-sm text-gray-500 max-w-md">
            Ajusta el periodo o quita filtros para ver resultados.
            También puedes volver al rango rápido de
