...
import { ArrowPathIcon, ArrowDownTrayIcon, NoSymbolIcon } from "@heroicons/react/24/outline";
...
const SkeletonCard = ({ height = 320 }: { height?: number }) => (
  <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-4">
    <div className="animate-pulse space-y-4">
      ...
      <div className="w-full bg-gradient-to-b from-gray-100 to-gray-200 rounded-xl" style={{ height }} />
    </div>
  </div>
);

const EmptyState = ({ onRetry, subtitle }: { onRetry: () => void; subtitle?: string }) => (
  <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-8 text-center">
    ...
    <p className="text-sm text-gray-500 mt-1">
      {subtitle ?? "Verifica que los sensores estén enviando lecturas o intenta cargar nuevamente."}
    </p>
...
const Dashboard: React.FC = () => {
  ...
  const kpis = useMemo(() => {
    ...
    sensors.forEach((s) => {
      ...
      const inRange = (val: number | null | undefined, min?: number, max?: number) => {
        ...
        const near = (edge?: number) => (edge == null ? false : Math.abs(val - edge) <= tol);
  ...
  return (
    <PageContainer title="Dashboard General" description={`Umbral conexión: ${CONNECTION_THRESHOLD_MIN} min`}>
      {/* Header de acciones */}
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-4 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <h2 className="text-base md:text-lg font-semibold text-gray-800">Tablero de Monitoreo</h2>
          <p className="text-xs text-gray-500">Sensores en vivo y estado por umbrales configurados</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto relative">
          {(isRefreshing || isLoading) && <div className="absolute inset-0 rounded-lg bg-white/60 backdrop-blur-[1px]" />}
          <Button onClick={handleRefresh} disabled={isRefreshing} title="Refrescar">
            <ArrowPathIcon className={["w-4 h-4", isRefreshing ? "animate-spin" : ""].join(" ")} />
            Refrescar
          </Button>
          <Button onClick={() => exportDashboard("image")} disabled={isRefreshing}>
            <ArrowDownTrayIcon className="w-4 h-4" />
            PNG
          </Button>
          <Button onClick={() => exportDashboard("pdf")} disabled={isRefreshing}>
            <ArrowDownTrayIcon className="w-4 h-4" />
            PDF
          </Button>
        </div>
      ...
      <div ref={dashboardRef} className="space-y-6">
        {isLoading ? (
          ...
          <EmptyState onRetry={handleRefresh} subtitle="No se encontraron sensores registrados." />
        ) : (
          <>
            <DashboardKPIs
